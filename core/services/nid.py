    ...
    def extract_text(self, image: Image.Image, prompt: str) -> str:
        ...
        try:
            ...
            OCRConfig.LLM_CONFIG["options"] = {
                ...
                "repeat_penalty": 1.8,  # > 1.0 -> Discourage repetition, < 1.0 -> encourage repetition
            ...
            logger.info(f"Raw VLM response: {result}...")  # Log first 500 chars for debugging
...
1. Bengali Name - Look for the "নাম" field in Bengali text
2. English Name - Look for the "Name" field in English text
3. Father's Name - Look for the "পিতা" field in Bengali text
4. Mother's Name - Look for the "মাতা" field in Bengali text
5. NID Number - Look for the "ID NO" field
6. Date of Birth - Look for the "Date of Birth" field

EXTRACTION RULES:
- Extract text EXACTLY as it appears on the card, character by character
- Do no repeat anything in the extracted texts
...
class NIDDataExtractor:
    ...
    def _parse_vlm_response(response: str) -> Dict:
        ...
        try:
            ...
            if json_str:
                ...
                logger.info(f"Successfully parsed VLM response with {len(parsed_data)} fields")
                logger.info(f"Final cleaned data: {parsed_data}")
                return parsed_data
    ...
    def get_extraction_feedback(self, extracted_data: Dict, side: NIDSide) -> Dict:
        ...
        try:
            if side == NIDSide.FRONT:
                expected_fields = ['english_name', 'bengali_name', 'father_name', 'mother_name', 'nid_no',
                                   'date_of_birth']
